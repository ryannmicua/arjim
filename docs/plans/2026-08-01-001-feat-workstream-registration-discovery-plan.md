...
1. Treat `VISION.md` and decisions marked `session-settled` as authoritative. VISION's Condition of Trust 5 (workspaces do not store live credentials in shared files) is an operator obligation the implementation does not police; session-settled operator decisions govern where they interpret VISION.
...
4. Treat `.workstream/manifest.json` as durable workspace authority and local inventory as replaceable projection state.
...
- R10. `.workstream/manifest.json` is the only v1 marker that makes a workspace a registration or discovery candidate.
...
- The v1 filesystem profile targets Windows NTFS and POSIX-compatible local filesystems. Owner-only enforcement for the projection uses the standard library on POSIX and the built-in `icacls` tool on Windows; where the declared profile's enforcement cannot be established, the projection fails closed before use. Network or synchronized filesystems are not assumed compliant, and the tested profile is recorded in the compatibility declaration.
...
- KTD2. **Use `.workstream/manifest.json` as the assistant-neutral marker.** The namespace stays portable and extensible while executable behavior remains in the Python package. **Supports R4, R10-R14.**
...
- KTD4. **Keep durable and device-local references separate.** Version 1 uses the literal workspace self-reference `.` and locates the marker at `.workstream/manifest.json` below the inspected root. Absolute paths, traversal, symlink-resolved paths, and local link locations belong only in implementation state. **Supports R2, R9, R13, R14.**
...
- **Verification:** The schemas are closed, versioned, and sufficient for every fixture planned by U1-U3; sufficiency is re-confirmed at U1, U2, and U3.
...
- **Approach:** Define a closed object with a required version field that readers dispatch on before applying the closed schema (v1 allowed value: `1`, JSON integer), lowercase RFC 4122 v4 UUID identity, required non-empty bounded label, required `direct|proxy` kind, literal `.` workspace reference, and a required non-empty list of bounded typed absolute record-source URIs. Enforce 256 KiB (262,144-byte) UTF-8, depth 8, 256-byte label, ASCII type token 64, ASCII record-source URI 2,048, and 1 to 32 record sources. Record-source URIs are RFC 3986 ASCII; non-ASCII content must be percent-encoded, so the 2,048-character limit is also a 2,048-byte limit. Accept syntactically valid URIs regardless of scheme or credential-bearing content; malformed URIs warn without invalidating the marker. Diagnostics never echo URI content or secrets; labels and local paths may be emitted.
- **Patterns to follow:** Stable identifiers and authority language in this plan; sensitive-data boundaries in `VISION.md`.
- **Test scenarios:** AE1, AE3, AE6, and AE9; missing and unknown fields; malformed UUID and kind; missing or unsupported marker version; device-path or traversal workspace references; duplicate record sources; controls and bidi overrides; field-limit cases at every boundary (256-byte label, 64-byte type token, 2,048-byte URI, 1 and 32 record sources); oversized raw input rejected at the read cap; malformed UTF-8, BOM, trailing content, duplicate keys, and depth 8/9 raw cases; instruction-like labels remain data.
...
- **Verification:** The contracts README carries an objective checklist — it names each contract file, the marker path (`.workstream/manifest.json`), the runner invocation (`python -m workstream_registration.conformance_runner`), the compliance bar, and the support profile, and every referenced path resolves — and a fresh implementer without access to this planning artifact can answer five factual questions (identity field name, marker path, runner command, compliance bar, support profile) from the contracts alone.
...
- **Approach:** Bind unregister confirmation to exact identity, stable target, and observed presence. Establish the declared cooperative-writer lock before deletion; if that cooperation cannot be established, return a non-success without deleting. Then re-read and compare after lock acquisition and again immediately before delete, delete, verify absence, and update projection. Implement `ProjectionInput` as immutable fields `{identity: UUID, label: string, marker_version: string, target_handle: bytes, workspace_path: local path, ordinal: integer}` and expose the projection boundary as `update(input) -> ProjectionResult`, where `ProjectionResult` is `{status: linked|registered-unlinked|projection-failed|conflict, identity, target_handle, ordinal}`. The idempotency key is `(identity, target_handle)`; same-key updates replace the local path and ordinal, while a conflicting identity for one captured target returns a conflict without changing the marker. Return `linked`, `registered-unlinked`, `conflict`, or a bounded projection failure without changing marker authority. Store projection rows transactionally in a private owner-only directory under per-user application data, enforce owner-only permissions on the database directory, database, and SQLite sidecars using the standard library on POSIX and the built-in `icacls` tool on Windows, verify them before use and after creation, and fail closed when the declared profile's enforcement cannot be established or pre-existing permissions are weaker. Keep the schema minimal: marker identity, label, target handle, local routing path/state, and deterministic input ordinal only; never copy record-source URI content or credentials. Support full rebuild as a transactional replacement from an explicit ordered list of workspace paths: each path means one workspace, no recursive traversal; symlink aliases deduplicate by target handle while retaining the first input's ordinal; expose that retained ordinal and input-order routing in the projection and stable `--json` result. Inaccessible or invalid paths return a non-success and leave the previous projection unchanged. Automatic discovery remains deferred. Treat projection failure as retriable local state.
...
| Traceability | U4 | The contracts README checklist passes: it names each contract file, the marker path, the runner invocation, the compliance bar, and the support profile; every referenced path resolves; a fresh implementer answers the five factual questions from the contracts alone. |
...
- **Filesystem profile target is resolved (2026-08-06)** — Version 1 targets Windows NTFS and POSIX-compatible local filesystems; owner-only enforcement uses the standard library on POSIX and the built-in `icacls` tool on Windows, failing closed when the declared profile's enforcement cannot be established. Open (non-blocking): U6 records the exact tested profile and patch versions in the compatibility declaration before compatibility is claimed.
...
### From the 2026-08-06 operator decision (review adoption)

- **Marker terminology reconciled; path superseded (2026-08-07)** — The canonical name of the durable artifact is **marker**, and its canonical path is now **`.workstream/manifest.json`** (R10, KTD2, KTD4, KTD13, U1). The 2026-08-06 reconciliation and the 2026-08-04 terminology decision asserted a different marker filename under `.workstream/` with a "do not reopen the name or path" clause; the **2026-08-07 operator decision** (digest `docs/session-digests/2026080702_marker_path_rename_manifest_json.md`) supersedes the filename part of those clauses: the marker is written at `.workstream/manifest.json`, while the term "marker" is retained. CONCEPTS.md's earlier "workstream manifest" drift is now adopted as the ruling rather than reverted. Historical records are not rewritten; this decision record makes the rename a ruling, not drift. The frozen schema sha256 changes with the schema description; the new hash is recorded in digest 2026080702.
- **Lock acquisition when the parent is absent** — When `.workstream` is absent, lock acquisition and parent creation are one atomic step: create the parent directory without replacement and exclusively create `.workstream/.registration.lock` within it before any marker write. A cancellation after parent-and-lock creation but before marker creation leaves an empty `.workstream` parent; a retry then requires a fresh inspection that captures the now-present parent identity and marker-absent state (the KTD6 no-transition confirmation variant). The KTD6 `ABSENT -> created-parent-identity` envelope transition and the U2 "interrupted parent-created retry" fixture already cover this case, so KTD6 and U2 need no further change.
- **v1 platform and owner-only enforcement** — Version 1 targets Windows NTFS and POSIX-compatible local filesystems under the declared filesystem profile. Owner-only enforcement for the projection uses the standard library on POSIX and the built-in `icacls` tool on Windows; where the declared profile's enforcement cannot be established, the projection fails closed before use. The compatibility declaration records the tested profile and the exact patch versions.
- **Marker schema closed-field decisions (freeze at U1)** — The marker carries a required version field (`version`, JSON integer, v1 allowed value `1`) that readers dispatch on before applying the schema; the identity is an RFC 4122 v4 lowercase UUID; the label is required and non-empty (256-byte cap unchanged); `kind` is required in the schema (`direct|proxy`) and the CLI defaults it to `direct` when omitted; the record-source list is required and non-empty (minimum 1, maximum 32 unchanged).
- **`register` on an existing marker** — `register` on a workspace that already holds a valid supported marker degrades to linking and reports `linked-existing` without confirmation (registration performs no write in that case).
- **Exit-code mapping is complete** — 0 for success (`registered`, `linked-existing`, `unregistered`, `invalid-marker-resolved`); 2 for operator stop or no write (`cancelled`, `stopped`); 3 for invalid or inaccessible input (including `occupied-invalid` inspection; the result vocabulary has no separate `invalid-marker` outcome, so malformed-marker inspection also reports `occupied-invalid`); 4 for conflict (`conflict`, `changed-marker-stopped`); 5 for partial/unverified completion (`written-unverified`, `registered-unlinked`, `invalid-deleted-unverified`); 6 for safe internal failure.
- **VISION trust rule precedence** — VISION's Condition of Trust 5 (no live credentials in shared workspace files) is an operator obligation the implementation does not police; session-settled operator decisions govern where they interpret VISION.

### From the 2026-08-07 operator decision (loop readiness)

- **U4 verification is an objective checklist** — The operator adopts the readiness-review mechanization (success-measure fix 2, 2026-08-06 review) as the bar for U4 and the Traceability gate: the contracts README must name each contract file, the marker path (`.workstream/manifest.json`), the runner invocation (`python -m workstream_registration.conformance_runner`), the compliance bar, and the support profile; every referenced path must resolve; and a fresh implementer must answer five factual questions (identity field name, marker path, runner command, compliance bar, support profile) from the contracts alone. **Changes:** U4 Approach and Verification line, Traceability gate row; DoD item 4 keeps its requirement-coverage wording.
- **Verification Contract gate count is 13, not 12** — The gate table has 13 rows: JSON integrity, Fixture structure, Protocol review, Result review, Fixture inventory, Traceability, Install and harness, Raw guard, Validation and diagnostics, Registration lifecycle, Unregister and projection, CLI and conformance, and Recovery and privacy (which spans U9-U11). Any "12 gates" statement was a miscount; the table is authoritative and is not changed.
- **README status flip is U11's terminal responsibility** — The repo `README.md` status line moves from "Planning. Not yet implemented" to the implemented state only after the complete project corpus passes, as part of U11's terminal step; until then U4's honesty rule governs the README. **Changes:** U11 Approach and Verification line; DoD item 12 already requires the final README content, so no DoD wording change.
