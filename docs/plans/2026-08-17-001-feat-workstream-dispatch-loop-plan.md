...
- **The operator chooses the dispatched agent's permission posture per dispatch, and it is recorded.** Model and mode are chosen per dispatch; the provider is pinned (KTD12). How much authority an agent has in a workspace is the operator's call, not a default Arjim picks silently. Governs R27, R28.
...
- R16. The job record is workspace-owned, durable, and the authority for what was requested: target workstream, instruction, confirmed permission posture (R27, R28), the declared outcome-note path, attribution (R3), creation time, and any `follows` link.
...
- R26. The instruction asks the agent to write an outcome note at a declared path beside its job record. Arjim reads that note when answering, passes it through the R29 code-point guard, cross-checks that the note's `job_id` equals the record's `job_id`, and reports its content as the agent's own claim. Every note reports one value from a closed note-status vocabulary that is orthogonal to the R8 job state and changes no job state: `present | absent | unreadable | schema-invalid | guard-failed | mismatched`. None of the non-`present` values is ever rendered as completion. A guard-failed note is not sanitized and re-rendered: it renders as "note present, unrenderable — open the agent" with a stable code, distinct from absent, because the operator must learn that a claim exists which Arjim will not repeat. A note whose `job_id` differs from the record's is `mismatched` and is never rendered, under that job or any other. The answer states its own observation time in UTC and renders each note's `reported_at` beside the claim it carries, so the operator judges a claim's age from stated times; Arjim declares no freshness window and never labels a note fresh or stale on the operator's behalf.
- R27. The dispatched agent's permission posture — the tuple of dispatch provider, model, and mode — is presented in the confirmation. Model and mode are chosen per dispatch by the operator; the provider is pinned (KTD12) and is not an operator choice. Arjim never selects the most permissive available model or mode on the operator's behalf, and never dispatches without a complete tuple recorded. The mode is validated against the live CLI at draft time, never a frozen enum.
- R28. The permission posture recorded in the job record — provider, model, and mode — is the one the operator confirmed. No path dispatches at a provider, model, or mode other than the confirmed ones.
...
- F2. **Ask how everything is going**
  ...
  - **Outcome:** Per-job state with honest unknowns, the agent's own claim where it left one, plus per-workspace change signal. Silence about a job is impossible; an unreachable substrate is named.
...
- AE17. **Covers R26, R9.** Given a job whose agent is `idle` and whose outcome note exists, the answer reports the note as the agent's own claim. Given a job whose agent is `idle` and whose note is absent, the answer reports `idle` with the outcome unknown and asserts no completion.
- AE18. **Covers R27, R28.** Given a dispatch whose confirmation named a restricted posture, the spawn uses that exact provider/model/mode tuple. No path selects the most permissive posture without it appearing in the confirmed text, and a dispatch attempted with no recorded posture is refused.
...
- Reading the dispatched agent's work product, transcript, or diffs to summarize or judge its outcome. Arjim reads only the declared outcome note (R26) and relays it as the agent's claim.
...
- **KTD2. Paseo is reached by CLI subprocess with `--json`, not by an MCP or HTTP client.** The repo has no HTTP client, MCP client, or async runtime in `src/`; it does have an established subprocess convention in `src/workstream_registration/projection.py:258-323` (fixed argv lists, an explicit timeout constant, `except (OSError, subprocess.TimeoutExpired)` converted to a bounded content-free error). Dispatch mirrors that convention with one Windows correction the `icacls` precedent never needed: the executable is resolved once at adapter init via `shutil.which`, and on Windows the npm shim resolves to a `.CMD` batch file, which `CreateProcess` cannot execute directly. U4 therefore invokes it through the command interpreter (`cmd.exe /c` with the fixed argv list) or resolves the shim to its node entry (`node <cli-js>`), settled and tested against the real binary during U4. A bare `paseo` name is never used in argv. Governs R6, R11, R23.
...
- **KTD11. The outcome note is a workspace file the agent writes, not a status Arjim infers.** Paseo reports no completion, so the honest options were to leave "did it finish?" unanswerable or to have the answer live where Arjim already reads. A note beside the job record costs one declared path, adds no watcher, and keeps the trust rule intact because Arjim reports it as the agent's claim rather than as verified truth. Job-record immutability (R19) is unaffected — the note is a sibling file, and R19 constrains Arjim's writes, not the agent's. Governs R26.
- **KTD12. Permission posture is an operator decision surfaced in the confirmation text, as a provider/model/mode tuple.** Paseo exposes modes such as `plan`, `default`, `acceptEdits`, `auto`, and `bypassPermissions`, but `AvailableModes` is provider-specific and operator-extensible, so the mode is an open string validated against the live CLI at draft time — never a frozen schema enum. The dispatch provider is pinned as a module constant (the operator's default workhorse provider, `deepseek/deepseek-v4-flash`); the model is recorded per dispatch, defaulting to that provider's default. An Arjim-chosen default would be Arjim deciding how much authority an agent gets in the operator's workspace, which `VISION.md:155-161` reserves to the operator. Recording the full tuple in the job record makes what was authorized auditable. Governs R27, R28.
...
- **Approach:**
  1. Define the job-record schema as a closed document: `schema_version` (int), `job_id`, `workstream_identity`, `instruction`, `permission_posture`, `outcome_note_path`, `follows` (optional job id), `actor`, `recorded_at`, `confirmation_ref`, `created_at`. Pin `job_id` as a uuid4-shaped token (lowercase hex with hyphens) with a regex constraint — it keys the U3 binding, the Paseo label value `arjim.job=<job_id>`, the `follows` reference, and the record filename. Cap `instruction` length explicitly; model `permission_posture` as a closed object with three required fields — `provider`, `model`, and `mode` — not a string enum (KTD12): `provider` is constrained to the pinned dispatch provider, `model` is a bounded non-empty string, and `mode` is an open non-empty string the schema does not enumerate because it is validated against the live CLI at draft time (U4); forbid additional properties on both the record and the posture object.
  1b. Define the outcome-note schema as a separate closed document the dispatched agent writes and Arjim only reads: `schema_version`, `job_id` (same pinned format as the record's), a bounded free-text `summary`, and a required ISO-8601 UTC `reported_at` timestamp that answers render beside the claim (R26). Nothing in it is trusted as verified truth; it is the agent's claim. The `summary` is an untrusted input passed through the dispatch-local guard at read time (U6, KTD13). Alongside it, define the note-status vocabulary as a third closed vocabulary in this version space — `present | absent | unreadable | schema-invalid | guard-failed | mismatched` — explicitly orthogonal to the R8 job-state enum: a note status is reported per job in addition to its job state and never substitutes for one, and no note status changes a derived job state. `guard-failed` is distinct from `absent` by design (the operator learns a claim exists that Arjim will not repeat, rendered with a stable code and never sanitized into a rendered claim), and `mismatched` means the note's `job_id` differs from its record's, which makes it unrenderable under any job.
...
- **Test scenarios:**
  ...
  - A record missing `permission_posture` is rejected; a posture whose provider is not the pinned one is rejected; a mode that is not a non-empty string is rejected.
  ...
  - The note-status fenced table parses and contains exactly the six R26 values, and no note-status value appears in the job-state table or vice versa.
...
- **Approach:**
  ...
  5. Provide a read-only outcome-note reader for the sibling path declared on a record, validating against the U1 outcome-note schema and returning absent for a missing, unreadable, or invalid note. This module never writes an outcome note — the dispatched agent does.
...
- **Approach:**
  1. Expose five operations: spawn a background labelled agent in a working directory at a caller-supplied provider/model/mode tuple, list all `arjim.job`-labelled agents in one batched `paseo ls --json -a -g` invocation, resolve a single agent by label, inspect an agent, and enumerate the pinned provider's available models and modes for draft-time posture validation.
  1b. Posture validation is live, not schematic: expose a bounded query against the real CLI (the provider-inspect/list-models surface) that returns the pinned provider's accepted models and `AvailableModes`, and have U5's draft path validate the operator's chosen model and mode against it before the confirmation is offered (R27, KTD12). An unreachable or unparseable validation query refuses the draft with a stable code rather than falling back to an assumed mode set — Arjim never dispatches at a mode it could not confirm the provider accepts.
  2. Build fixed argv lists only — never a shell string — with an explicit module-level timeout constant, mirroring the icacls call shape. Resolve the executable once at init via `shutil.which`; on Windows the resolved npm shim is a `.CMD` batch file, so invoke it through the command interpreter (`cmd.exe /c` with the fixed argv list) or resolve the shim to its node entry (`node <cli-js>`) — settled and tested against the real binary on this machine (KTD2). Every spawn passes a fixed, content-free `--title` (e.g. `Arjim job <job-id>`) so Paseo never derives a title from instruction text (R23). Settle Q4 here: determine against the real binary whether an end-of-options separator stops flag parsing, and if it does not, refuse option-shaped instructions outright. The instruction must be unable to alter the spawn's own flags (R30).
...
- **Test scenarios:**
  ...
  - On Windows, the constructed argv never uses a bare `paseo` name: it carries the `shutil.which`-resolved path (or node entry), and a live-machine probe succeeds against the real CLI (KTD2).
  ...
  - The posture-validation query returns the pinned provider's models and modes from the live CLI, and a mode absent from that live set is refused at draft time rather than matched against a frozen enum; an unreachable validation query refuses rather than assuming a mode set.
  - The spawn passes the caller-supplied provider/model/mode tuple — the confirmed provider and model appear in the constructed argv — and refuses to construct argv when any part of the tuple is missing (`Covers AE18`).
- **Verification:** Every failure mode of the external CLI produces exactly one bounded unreachable value; no test observes raw external output escaping the adapter; no instruction content can reach the argv as a flag.
...
- **Approach:**
  ...
  1b. Carry the provider/model/mode posture tuple through the draft so it appears in the confirmed text, and refuse to draft without a complete tuple (R27, R28).
...
- **Test scenarios:**
  ...
  - The confirmed provider/model/mode tuple is the tuple handed to the adapter, and a draft without a complete tuple is refused (`Covers AE18`).
...
- **Approach:**
  ...
  2b. Read the outcome note beside each job record when present and resolve exactly one note status from the U1 vocabulary: validate against the U1 note schema, apply the dispatch-local guard (KTD13), and cross-check the note's `job_id` against the record's. A note whose `job_id` differs is `mismatched` and its content is never rendered — not under that job and not under the job it names, because a note is only evidence where it sits. A note that fails the guard is `guard-failed`: it is reported as note present but unrenderable with a stable code, distinct from `absent`, and is never sanitized, escaped, or partially rendered. Only a `present` note is attached as the agent's own claim, and it is attached with the note's `reported_at`. No note status alters the derived job state or adds completion language (R26).
...
- **Test scenarios:**
  ...
  - An `idle` job with a valid outcome note reports the note as the agent's claim; an `idle` job with no note reports the outcome as unknown (`Covers AE17`).
...
- The operator can ask Arjim to get something done in a registered workstream, see the exact instruction and the agent's permission posture before anything runs, confirm both, and have a Paseo agent working in that workspace.
...
| Finding | Applied in |
|---|---|
| P0: bare-name `paseo` fails on Windows (`.CMD` shim) | KTD2, U4: `shutil.which` resolution; shim invoked via `cmd.exe /c` or resolved to its node entry; live-machine test |
