...
- **What it does:** inspects the workspace, drafts the marker (the manifest file at `.workstream/manifest.json`), generating a fresh identity, previews the exact draft, and writes it only after exact confirmation. The write is create-only; after the write the file is reopened, re-validated, and the identity verified (read-back); only then is the projection linked.
