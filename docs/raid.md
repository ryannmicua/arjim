...
|---|---|---|---|---|---|---|
| R-001 | Contract without executable proof | High | Medium | Arjim CE | 2026-08-01 | A malformed schema or contradictory fixture can survive manual review. Mitigation: one expectation manifest, every fixture declares its expected result, runtime selection includes a standards-compliant validator. |
...
| R-005 | Secret leakage | High | Medium | Arjim CE | 2026-08-01 | URI syntax can hide credentials. Mitigation: KTD9 accepts credential-bearing URIs as untrusted data, never echoes sensitive values, and warns on malformed ones; later adapters own secret scanning. |
| R-006 | Schema lock-in | Medium | Medium | Arjim CE | 2026-08-01 | A closed provider enum would force schema changes per integration. Mitigation: keep record-home type tokens open and version behavior, not providers. |
...
| A-003 | Record homes are typed, absolute URI references validated for structure only; any syntactically valid URI is accepted regardless of scheme or credential-bearing components, and malformed ones warn. Provider access and ownership checks require later adapters. | Arjim CE | 2026-08-01 | Reflects KD10 resolution. |
| A-004 | Record-home URIs are accepted as untrusted data even when credential-bearing; adapters never dereference them and diagnostics never echo them. Later adapters own secret scanning. | Arjim CE | 2026-08-01 | Reflects KD10 resolution. |
| A-005 | Unregister uses the same operator-confirmation authority as registration; no adapter deletes a marker without a confirmed exact identity. | Arjim CE | 2026-08-01 | Reflects KD9 / KTD10. |
...
|---|---|---|---|---|
| D-001 | Future runtime and validation library selection. | Arjim CE | 2026-08-01 | Runtime-neutral contract is delivered first; adapters come after runtime selection (KTD1). |
