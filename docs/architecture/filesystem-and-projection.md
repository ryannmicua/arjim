...
- **Windows:** the built-in `icacls` tool — reset inheritance (`/inheritance:r`) and grant only the current user, then parse-verify the ACL (`_enforce_owner_only_windows`, `projection.py:199-224`; `_verify_owner_only_windows`, `projection.py:227-275`). Directory `(OI)(CI)` inheritance makes the sidecars inherit the owner-only ACL; sidecars are verified with `allow_inherited=True`, everything else strictly (`projection.py:356-364`). Known limitation: the grant is applied before parse-verification, so if verification fails (e.g. unexpected or localized `icacls` output), enforcement fails closed with the path already left inheritance-disabled and owner-only — the target state is applied, only the verification is uncertain; recovery is re-running enforcement (idempotent — re-apply + re-verify) or applying `icacls` manually.
